      ...
      <div className="flex w-full max-w-sm flex-col items-center gap-8">
        ...
        <div className="w-full rounded-2xl border bg-card p-6 shadow-sm">
          <form onSubmit={handleSubmit} className="flex flex-col gap-3">
            {googleConflict && (
              <div className="flex flex-col gap-2 rounded-lg bg-amber-50 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-800 px-3 py-2.5">
                <p className="text-xs text-amber-700 dark:text-amber-400">
                  This email is already linked to a Google account. Sign in with Google, then set a password from your account settings.
                </p>
                <button
                  type="button"
                  onClick={() => signIn("google", { callbackUrl: "/account" })}
                  className="flex items-center justify-center gap-2 rounded-lg border bg-background px-3 py-2 text-xs font-medium transition-colors hover:bg-muted"
                >
